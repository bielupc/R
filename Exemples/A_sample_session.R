# A sample session
#
# The following session is intended to introduce 
# to you some features of the R environment by
# using them. Many features of the system will be 
# unfamiliar and puzzling at first, but this
...
# (Within R, the prompt on the left hand side 
# will notbe shown to avoid confusion.)

help.start()
# Start the HTML interface to on-line help 
# (using a web browser available at your machine). 
# You should briefly explore the features of this facility 
# with the mouse.
# Iconify the help window and move on to the next part.

x <- rnorm(50)
y <- rnorm(x)
# Generate two pseudo-random normal vectors 
...
# A standard regression diagnostic plot 
# to check for heteroscedasticity. Can you see it?

qqnorm(resid(fm), main="Residuals Rankit Plot")
# A normal scores plot to check for skewness, 
# kurtosis and outliers. (Not very useful here.)

rm(fm, fm1, lrf, x, dummy)
# Clean up again.


# The next section will look at data from 
# the classical experiment of Michaelson and Morley
# to measure the speed of light. 
# This dataset is available in the 'morley' object, 
# but we will read it to illustrate the 'read.table' function.
...
mm 
# Read in the Michaelson and Morley data as a data frame, 
# and look at it. There are five experiments (column Expt) 
# and each has 20 runs (column Run) and sl is the
...
summary(fm)
# Analyze as a randomized block, 
# with runs' and 'experiments' as factors.

fm0 <- update(fm, . ~ . - Run)
anova(fm0, fm)
# Fit the sub-model omitting 'runs', and compare using 
...
# We now look at some more graphical features: 
...
# f is a square matrix, with rows and columns indexed 
# by x and y respectively, of values of the function 
# cos(y)/(1 + x2).

oldpar <- par(no.readonly = TRUE)
par(pty="s")
# Save the plotting parameters and 
...
# (of which you can get hardcopies if you wish),
# ...
objects(); rm(x, y, f, fa)
...
par(pty="s")
plot(z, type="l")
# Plotting complex arguments means 
# plot imaginary versus real parts. 
# This should be a circle.

w <- rnorm(100) + rnorm(100)*1i
# Suppose we want to sample points within the unit circle. 
# One method would be to take complex numbers with 
# standard normal real and imaginary parts ...
...
lines(z)
# All points are inside the unit circle, 
...
q() 
# Quit the R program. 
# You will be asked if you want to save the R workspace, 
# and for an exploratory session like this, 
